...
class testATAnew(c: AllToAll) extends PeekPokeTester(c) {
  ...
  testNumberCycles(3,6)
  //testNumberCycles(3,2)
  //testNumberCycles(3,3)
...
class testATAnew1(c: AllToAll) extends PeekPokeTester(c) {
  ...
  def testNumberCycles(n: Int, dim_N: Long):Int ={
...
class AllToAllTest extends ChiselFlatSpec {
  ...
  behavior of "test_scala_n_1"
  it should "test scala" in {
    chisel3.iotesters.Driver.execute( testerArgs, () => new AllToAll()) {
      c => new testATAnew(c)
    } should be (true)
  }

  behavior of "test_scala_n_2"
  it should "test scala" in {
    chisel3.iotesters.Driver.execute( testerArgs, () => new AllToAll()) {
      c => new testATAnew1(c)
    } should be (true)
