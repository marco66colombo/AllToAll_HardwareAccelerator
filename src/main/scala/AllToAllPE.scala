...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  val index_calcualtor = Module(new IndexCalculatorV1(n,n*n,log2Up(n)))

  val read_values = Reg(Vec(4, Bits(64.W)))
  val read_values_valid = RegInit(VecInit(Seq.fill(4)(false.B)))
  val read_x_dest = Reg(Vec(4,UInt(log2Up(n).W)))
  val read_y_dest = Reg(Vec(4,UInt(log2Up(n).W)))
  ...
  val generation_dispatcher_0 = Module(new GenerationDispatcher(log2Up(n)))
  val generation_dispatcher_1 = Module(new GenerationDispatcher(log2Up(n)))
  val generation_dispatcher_2 = Module(new GenerationDispatcher(log2Up(n)))
  val generation_dispatcher_3 = Module(new GenerationDispatcher(log2Up(n)))
  ...
  val left_mux = Module(new MyPriorityMux(log2Up(n)))
  val right_mux = Module(new MyPriorityMux(log2Up(n)))
  val up_mux = Module(new MyPriorityMux(log2Up(n)))
  val bottom_mux = Module(new MyPriorityMux(log2Up(n)))
  ...
  */
  ...
  left_out_arbiter.io.in(1).valid := false.B//right_dispatcher.io.left && right_in.valid
  left_out_arbiter.io.in(2).bits <> up_in.bits
  left_out_arbiter.io.in(2).valid := false.B//up_dispatcher.io.left && up_in.valid
  left_out_arbiter.io.in(3).bits <> bottom_in.bits
  left_out_arbiter.io.in(3).valid := false.B//bottom_dispatcher.io.left && bottom_in.valid
  ...
  right_out_arbiter.io.in(1).valid := false.B//left_dispatcher.io.right && left_in.valid
  right_out_arbiter.io.in(2).bits <> up_in.bits
  right_out_arbiter.io.in(2).valid := false.B//up_dispatcher.io.right && up_in.valid
  right_out_arbiter.io.in(3).bits <> bottom_in.bits
  right_out_arbiter.io.in(3).valid := false.B//bottom_dispatcher.io.right && bottom_in.valid
  ...
  up_out_arbiter.io.in(1).valid := false.B//left_dispatcher.io.up && left_in.valid
  up_out_arbiter.io.in(2).bits <> right_in.bits
  up_out_arbiter.io.in(2).valid := false.B//right_dispatcher.io.up && right_in.valid
  up_out_arbiter.io.in(3).bits <> bottom_in.bits
  up_out_arbiter.io.in(3).valid := false.B//bottom_dispatcher.io.up && bottom_in.valid
  ...
  bottom_out_arbiter.io.in(1).valid := false.B//left_dispatcher.io.bottom && left_in.valid
  bottom_out_arbiter.io.in(2).bits <> right_in.bits
  bottom_out_arbiter.io.in(2).valid := false.B//right_dispatcher.io.bottom && right_in.valid
  bottom_out_arbiter.io.in(3).bits <> up_in.bits
  bottom_out_arbiter.io.in(3).valid := false.B//bottom_dispatcher.io.bottom && bottom_in.valid
  ...
  when(stateAction === idle1){
    ...
    when(do_read && !index_calcualtor.io.last_iteration){
      ...
      when(this_PE_generation_0){
        memPE(index_write_this_PE) := read_values(0)
      }
      when(this_PE_generation_1){
        memPE(index_write_this_PE) := read_values(1)
      }
      when(this_PE_generation_2){
        memPE(index_write_this_PE) := read_values(2)
      }
      when(this_PE_generation_3){
        memPE(index_write_this_PE) := read_values(3)
      }
